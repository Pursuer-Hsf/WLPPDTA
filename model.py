...
class DTIDenseNet3to1(nn.Module):
...
class DTIDenseNet3to2(nn.Module):
...
class DTIDenseNet2to1(nn.Module):
...
def test(model: nn.Module, test_loader, loss_function, device, path=None, save=False):
    ...
    if save:
        save_xlt(path=path, names=names, targets=targets, outputs=[outputs, outputs_ss, outputs_ps], 
                 multiout=isinstance(model, DTIDenseNet3to2) or isinstance(model, DeepDTAF_3to2))
